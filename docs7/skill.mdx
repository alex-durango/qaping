...
The CLI also hosts builds and discovers saved rounds with `qaping rounds --all`.
...
come back yet?" — check BOTH server rounds and local workflow state:

1. Run `qaping rounds --all --game <game_slug> --json`, using the plan's slug.
   Follow `next_cursor` with `--cursor <next_cursor>` until the relevant
   history is covered. Pages hold 50 rounds by default (`--limit 1-100`).
   Omit `--game` to search the whole account, including older rounds filed
   without a slug.
2. Read `qa-open-rounds.json` and `qa-rounds.jsonl` at the repo root. Match by
   `ping_id` (the history ledger calls it `round_id`). A server round absent
   from the open file is NOT a new round to file: recover its existing ID.
   A round already in the history ledger has been collected; do not append
   or report it twice merely because it remains in the server listing.
3. Fetch outstanding results with `qaping results <ping_id>` or
   `qaping_results`. Recover game/build/filing time from the listing and
   check IDs from the actual results and plan. Do not invent lost patch
   notes or coverage. Restore open records only for work still owed by THIS
   repo, preserving existing notes and checks.

`--all` (`--remote` also works) reads the server even from a fresh directory;
it never writes local files or renews a lease. It includes pending, complete
and expired rounds. A network or login error is NOT an empty account: resolve
it before filing replacements. The listing contains only the current login's
playtests and may include other games: identify this repo's rounds by game or
build before adopting them.

Bare `qaping rounds` still lists the local file with live status; without
the CLI, call `qaping_results` once per recorded `ping_id`, but that alone
cannot discover missing IDs. The open file is this, and nothing else:

```json
{"rounds": [{"ping_id": "<uuid>", "filed_at": "<ISO>", "platform": "windows",
  "build": "<the /b/<slug> or store URL>", "est_minutes": 10,
  "deadline_seconds": 86400, "game": "solar-drift", "checks": ["boot-to-menu"],
...
0. **Read the board.** `qaping_issues` with the plan's `game_slug` (action
   ...
   is the bug half of "what happened since last time"; the ledger's last line
   is the check half.
...
8. **Report** from `qaping_results`:
   ...
   - **Ledger append (MUST).** Append ONE JSON line to `qa-rounds.jsonl` at
     ...
     "new": [7, 8], "unreached": [9], "still_open": [2]}, "verdict": …,
     "credits": …, "follow_ups": […]}`. The `issues` object is the board's
     movement this round, by seq (omit it entirely when the round carried no
     `game`), and `follow_ups` is NON-BUG follow-ups only from now on — a
     check to add, a build affordance to write, a question to settle. A bug
     goes on the board, where it is numbered, re-checked and closable; free
     text nobody re-reads is where bugs went to die. `report_url` is the
     round's service report page (the `report_url` the results tool returns —
     never a PR or issue link). No evidence-link field, deliberately: signed
     recording URLs expire in days, so durable evidence pointers live in the
     report/issues, not the ledger. The ledger is machine truth;
