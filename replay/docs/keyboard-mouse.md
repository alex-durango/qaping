...
Replay saves your physical input and can replay that same session while making
...
This walkthrough uses the **0.2.4 source snapshot**. npm still serves 0.1.0,
which does not include keyboard/mouse support.

## The easy way: desktop controls

Download this repository with **Code → Download ZIP**, extract it, open the **replay** folder, and
double-click **Start Replay.cmd**. It finds Node.js or downloads a verified local
...
Switching focus stops a session. Replay does not pull focus back from another app.
...
size and pause/menu state. Check against your starting screenshot. Replay
...
to let Replay launch it; wait at Start until its save and menus are ready.
...
| --- | --- |
| The launcher does not open | Extract the ZIP first, then double-click Start Replay.cmd inside the extracted folder. First start needs an internet connection; a setup error stays visible in the launcher window. |
...
| F8 appears not to stop | Check any keyboard Fn/F-lock behavior. Switch back to Qaping Replay and click Stop if it is still active; leaving the game also ends input. Let it finish saving. |
| The controls disappear | During play the control panel minimizes. It should return after completion. Reopen Start Replay.cmd if the app closed; your saved sessions remain on disk. |
...
| F8 appears not to stop | Check game focus and any keyboard Fn/F-lock behavior. If needed, Ctrl+C in Replay terminal requests a stop; wait for its final output. |
...
[Report a problem](https://github.com/alex-durango/qaping/issues) with the
